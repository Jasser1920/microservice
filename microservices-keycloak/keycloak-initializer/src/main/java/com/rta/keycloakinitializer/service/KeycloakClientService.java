...
public class KeycloakClientService {
    ...
    private static final String FRONTEND_CLIENT_ID = System.getenv("FRONTEND_CLIENT_ID");
    ...
    public void createRealmAndConfigs(String realmName) {
        if(!realmExists(realmName)) {
            ...
            // Create backend client representation for hotel microservices
            ...
            // Hotel-specific roles: CLIENT (customer), STAFF (employees), MANAGER, ADMIN
            createClient(realmName, backendClient, List.of("CLIENT", "STAFF", "MANAGER", "ADMIN"));
            String clientBackendId = getClientId(realmName, BACKEND_CLIENT_ID);
            compositeClientRoleWithRealmRole(realmName, clientBackendId, "CLIENT", "APP_USER");
            ...
            createClient(realmName, gatewayClient, List.of("CLIENT"));
            String gatewayClientId = getClientId(realmName, GATEWAY_CLIENT_ID);
            compositeClientRoleWithRealmRole(realmName, gatewayClientId, "CLIENT", "APP_USER");

            // Create frontend-client representation (public SPA)
            ClientRepresentation frontendClient = new ClientRepresentation();
            String urlFrontend = FRONTEND_URL != null ? FRONTEND_URL : "http://localhost:8091";
            String frontendClientId = FRONTEND_CLIENT_ID != null ? FRONTEND_CLIENT_ID : "frontend-client";
            frontendClient.setClientId(frontendClientId);
            frontendClient.setRootUrl(urlFrontend);
            frontendClient.setWebOrigins(List.of(urlFrontend));
            frontendClient.setRedirectUris(List.of(urlFrontend + "/*"));
            frontendClient.setPublicClient(true);
            frontendClient.setEnabled(true);
            frontendClient.setStandardFlowEnabled(true);

            createClient(realmName, frontendClient, List.of("CLIENT"));
            String frontendClientUuid = getClientId(realmName, frontendClientId);
            compositeClientRoleWithRealmRole(realmName, frontendClientUuid, "CLIENT", "APP_USER");

            addGroupsToToken(realmName, "profile");

            createGroup(realmName, "PROVIDERS");

            // Create hotel management users
            createUser(realmName, "admin", "admin123", "admin@hotel.com");
            assignRoleToUser(realmName, "admin", clientBackendId, "ADMIN");

            createUser(realmName, "manager", "manager123", "manager@hotel.com");
            assignRoleToUser(realmName, "manager", clientBackendId, "MANAGER");

            createUser(realmName, "staff", "staff123", "staff@hotel.com");
            assignRoleToUser(realmName, "staff", clientBackendId, "STAFF");

            createUser(realmName, "client1", "client123", "client1@email.com");
            assignRoleToUser(realmName, "client1", clientBackendId, "CLIENT");

            createUser(realmName, "client2", "client123", "client2@email.com");
            assignRoleToUser(realmName, "client2", clientBackendId, "CLIENT");
    ...
    public void compositeClientRoleWithRealmRole(String realmName, String clientId, String clientRole, String realmRole){
        try {
            ...
            RoleRepresentation realmRoleRepr = realmRoleResource.toRepresentation();
            String realmRoleId = realmRoleRepr.getId();
            ...
            System.out.println("Composite role created: " + clientRole + " -> " + realmRole);
        } catch (Exception e) {
            System.err.println("Error creating composite role " + clientRole + " with realm role " + realmRole + ": " + e.getMessage());
            e.printStackTrace();
        }
    }
